...
export class Admin implements OnInit {
  ...
  // Bloom Plan deliveries normally get generated by the 8am daily job — this
  // lets admin run that same pass on demand (e.g. right after a customer
  // subscribes, to confirm it's routing to the right merchant immediately).
  generatingSubscriptionOrders = signal(false);

  generateSubscriptionOrdersNow(): void {
    this.runSubscriptionGeneration(false);
  }

  async forceGenerateSubscriptionOrdersNow(): Promise<void> {
    const ok = await this.confirmService.ask({
      title: 'Force-generate ALL active subscriptions?',
      message: 'This ignores delivery dates and generates a real order for EVERY active Bloom Plan right now — mainly for testing merchant routing. It also advances each one\'s next delivery date, shifting their real schedule forward. Only do this if you understand that trade-off.',
      confirmText: 'Force generate', danger: true,
    });
    if (!ok) return;
    this.runSubscriptionGeneration(true);
  }

  private runSubscriptionGeneration(force: boolean): void {
    this.generatingSubscriptionOrders.set(true);
    this.http.post<any>(`${environment.apiUrl}/api/admin/subscriptions/generate?token=${this.token}&force=${force}`, {}).subscribe({
      next: (r) => {
        this.generatingSubscriptionOrders.set(false);
        this.toastService.show(`Checked ${r.checked} · Generated ${r.generated} · Flagged ${r.flagged}${r.errors ? ' · Errors ' + r.errors : ''}`);
        this.loadSubscriptions();
        this.loadOrders();
      },
      error: (err) => { this.generatingSubscriptionOrders.set(false); this.toastService.show(err.error?.detail || 'Generation failed', 'error'); },
    });
  }
